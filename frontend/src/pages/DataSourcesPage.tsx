...
const DataSourcesPage: React.FC = () => {
  ...
  const testConnectionMutation = useMutation({
    ...
    onSuccess: (response, variables) => {
      const dataSourceId = variables.dataSourceId as string;
      setTestResults(prev => ({
        ...prev,
      ...
      <div className="sm:flex sm:items-center sm:justify-between">
        <div className="flex-1 min-w-0">
          <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl sm:truncate">
            <Database className="inline h-8 w-8 mr-2 text-blue-600" />
            Sistemas de Origen
          </h1>
      ...
      <div className="bg-white rounded-lg shadow-md">
        ...
        {dataSources?.data?.data?.length === 0 ? (
          ...
          <div className="divide-y divide-gray-200">
            {(dataSources?.data?.data as DataSource[])?.map((dataSource: DataSource) => (
